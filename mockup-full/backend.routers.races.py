...
CRUD for /races, plus the bulk weekend upsert endpoint.
"""
...
def list_sessions(race_id: str, db: Session = Depends(get_session)):
    ...
    for s in sessions:
        entries = db.exec(
            select(SessionEntry).where(SessionEntry.session_id == s.id)
        ).all()
        result.append(SessionRead(
            id=s.id,
            race_id=s.race_id,
            session_type=s.session_type,
            entries=[SessionEntryRead.model_validate(e) for e in entries],
        ))
    return result
...
def upsert_weekend(
    ...
    """
    Replace all session data for a race weekend in one call.
    Existing sessions for this race are deleted and recreated.
